...
typedef struct __attribute__((packed)) HLDVariables {
  ...
  /*
   * Not certain what this address even references, but the custom events won't
   * work unless some of their fields point to this address.
   */
...
typedef struct __attribute__((packed)) HLDFunctions {
  ...
  int32_t (*actionSpriteAdd)(const char *fname, size_t imgNum, int32_t unknown0,
                             int32_t unknown1, int32_t unknown2,
                             int32_t unknown3, uint32_t origX, uint32_t origY);
  /* Overwrite an existing sprite with a new one. */
  void (*actionSpriteReplace)(int32_t spriteIdx, const char *fname,
                              size_t imgNum, int32_t unknown0, int32_t unknown1,
