	...
	({ \
		...
		if ( \
			...
			if (FoxArrayMEmpty(Mod *, &modman.context)) { \
				LogErr( \
						"Key overflow while constructing internal configuration key. " \
				...
				LogErr( \
						"Key overflow while constructing configuration key for mod " \
...
static toml_datum_t ParseValueFromArray(
		toml_array_t * array,
		uint32_t idx,
		ConfType * type
) {
	...
	/* Boolean. */
	if ((result = toml_bool_at(array, idx)).ok) *type = CONF_BOOL;
	/* Integer. */
	else if ((result = toml_int_at(array, idx)).ok) *type = CONF_INT;
	/* Double. */
	else if ((result = toml_double_at(array, idx)).ok) *type = CONF_DOUBLE;
	/* String. */
	else if ((result = toml_string_at(array, idx)).ok) *type = CONF_STRING;
	/* Invalid. */
	else {
		LogErr(
...
static void ParseArray(toml_array_t * array) {
	...
	for (uint32_t idx = 0; idx < numElems; idx++) {
		...
		if (idx == 0) {
			switch (entType) {
				case CONF_BOOL: entArr = FoxArrayMNewExt(bool, numElems); break;
				case CONF_INT: entArr = FoxArrayMNewExt(int64_t, numElems); break;
				case CONF_DOUBLE: entArr = FoxArrayMNewExt(double, numElems); break;
				case CONF_STRING: entArr = FoxArrayMNewExt(char *, numElems); break;
				default: break;
			}
		}
		...
		switch (entType) {
			case CONF_BOOL: *FoxArrayMPush(bool, entArr) = datum.u.b; break;
			case CONF_INT: *FoxArrayMPush(int64_t, entArr) = datum.u.i; break;
			case CONF_DOUBLE: *FoxArrayMPush(double, entArr) = datum.u.d; break;
			case CONF_STRING: *FoxArrayMPush(char *, entArr) = datum.u.s; break;
			default: break;
		}
	}
...
static void ParseTable(toml_table_t * table) {
	...
	for (uint32_t idx = 0; ; idx++) {
		if (!(key = toml_key_in(table, idx))) break;
		PushKey(key);

		/* Table. */
		if ((nextTable = toml_table_in(table, key))) ParseTable(nextTable);
		/* Array. */
		else if ((nextArray = toml_array_in(table, key))) ParseArray(nextArray);
		/* Value. */
		else ParseValueFromTable(table, key);
...
static bool ConfEntryDeinitCallback(
		ConfEntry * entry,
		void * ctx
) {
	(void)ctx;
...
AER_EXPORT bool AERConfManGetBool(const char * key) {
	...
	ErrIf(
			entry->type != CONF_BOOL || entry->isArray,
			AER_FAILED_PARSE,
			false
	);
...
AER_EXPORT size_t AERConfManGetBools(
		const char * key,
		size_t bufSize,
		bool * boolBuf
) {
	...
	ConfType type = entry->type;
	ErrIf(
			!(type == CONF_BOOL || type == CONF_NULL) || !entry->isArray,
			AER_FAILED_PARSE,
			0
	);
	if (type == CONF_NULL) return 0;
...
AER_EXPORT int64_t AERConfManGetInt(const char * key) {
	...
	ErrIf(!entry, AER_FAILED_LOOKUP, 0);
	ErrIf(
			entry->type != CONF_INT || entry->isArray,
			AER_FAILED_PARSE,
			0
	);
...
AER_EXPORT size_t AERConfManGetInts(
		const char * key,
		size_t bufSize,
		int64_t * intBuf
) {
	...
	ConfType type = entry->type;
	ErrIf(
			!(type == CONF_INT || type == CONF_NULL) || !entry->isArray,
			AER_FAILED_PARSE,
			0
	);
	if (type == CONF_NULL) return 0;
...
AER_EXPORT double AERConfManGetDouble(const char * key) {
	...
	ErrIf(!entry, AER_FAILED_LOOKUP, 0);
	ErrIf(
			entry->type != CONF_DOUBLE || entry->isArray,
			AER_FAILED_PARSE,
			0
	);
...
AER_EXPORT size_t AERConfManGetDoubles(
		const char * key,
		size_t bufSize,
		double * doubleBuf
) {
	...
	ConfType type = entry->type;
	ErrIf(
			!(type == CONF_DOUBLE || type == CONF_NULL) || !entry->isArray,
			AER_FAILED_PARSE,
			0
	);
	if (type == CONF_NULL) return 0;
...
AER_EXPORT const char * AERConfManGetString(const char * key) {
	...
	ErrIf(!entry, AER_FAILED_LOOKUP, 0);
	ErrIf(
			entry->type != CONF_STRING || entry->isArray,
			AER_FAILED_PARSE,
			0
	);
...
AER_EXPORT size_t AERConfManGetStrings(
		const char * key,
		size_t bufSize,
		const char ** strBuf
) {
	...
	ConfType type = entry->type;
	ErrIf(
			!(type == CONF_STRING || type == CONF_NULL) || !entry->isArray,
			AER_FAILED_PARSE,
			0
	);
	if (type == CONF_NULL) return 0;
